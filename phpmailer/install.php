...
// Create tables
rex_sql_table::get(rex::getTable('project_manager_domain_phpmailer'))
...
$cronjob->setQuery('SELECT id FROM '.rex::getTable('cronjob'). ' WHERE type LIKE "rex_cronjob_project_manager_phpmailer"');
...
if ($cronjob->getRows() == 0) {
  ...
  $cronjob->setValue('name','Projekt Manager: PHPMailer Daten');
  $cronjob->setValue('description','');
  $cronjob->setValue('type','rex_cronjob_project_manager_phpmailer');
  ...
  try {
    ...
    echo rex_view::success('Der Cronjob "Projekt Manager: PHPMailer Daten" wurde angelegt. ');
  } catch (rex_sql_exception $e) {
    echo rex_view::warning('Der Cronjob "Projekt Manager: PHPMailer Daten" wurde nicht angelegt.<br/>Wahrscheinlich existiert er schon.');
  }
}